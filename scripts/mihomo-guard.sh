...
#
# Requires: docker exec access to mihomo-tun container
# ============================================================
...
ACTIVE=$(docker exec "$MIHOMO" curl -s http://127.0.0.1:9090/proxies/$GROUP 2>/dev/null     | python3 -c "import sys,json; print(json.load(sys.stdin).get('now',''))" 2>/dev/null || echo "")
...
PROXIES=$(docker exec "$MIHOMO" curl -s http://127.0.0.1:9090/proxies/$GROUP 2>/dev/null     | python3 -c "
import sys, json
